...
export const getCounterLedgerState = async (
  ...
  const state = await providers.publicDataProvider
    ...
    .then((contractState) => (contractState != null ? Counter.ledger(contractState.data).assetName : null));
...
export const proveOwnership = async (counterContract: DeployedCounterContract): Promise<FinalizedTxData> => {
  logger.info("Verifying ownership via challenge...");
  const finalizedTxData = await counterContract.callTx.proveOwnership();
  logger.info(`Ownership proven in tx ${finalizedTxData.public.txId} (block ${finalizedTxData.public.blockHeight})`);
  return finalizedTxData.public;
};

export const transferZkAsset = async (
  counterContract: DeployedCounterContract,
  newOwner: Uint8Array
): Promise<FinalizedTxData> => {
  logger.info(`Transferring zkAsset to new owner`);
  const finalizedTxData = await counterContract.callTx.transferAsset(newOwner);
  logger.info(`Purchase tx ${finalizedTxData.public.txId} added to block ${finalizedTxData.public.blockHeight}`);
  return finalizedTxData.public;
};


export const burnZkAsset = async (counterContract: DeployedCounterContract): Promise<FinalizedTxData> => {
  logger.info('Burning zkAsset...');
  const finalizedTxData = await counterContract.callTx.burnAsset();
...
export const displayCounterValue = async (
  providers: CounterProviders,
  counterContract: DeployedCounterContract,
): Promise<{ counterValue: string | null; contractAddress: string }> => {
...
export const configureProviders = async (wallet: Wallet & Resource, config: Config) => {
  ...
  return {
    ...
    zkConfigProvider: new NodeZkConfigProvider<'proveOwnership'>(contractConfig.zkConfigPath),
