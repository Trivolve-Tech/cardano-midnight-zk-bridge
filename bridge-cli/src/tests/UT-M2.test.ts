...
describe('API', () => {
  ...
  it('UT-M2 · Proof Format Check', async () => {
    ...
    const foundZkAssetContract = await api.joinContract(providers, zkAssetContract.deployTxData.public.contractAddress, {
      assetOwnerSecretKey: Buffer.from("0".repeat(64), "hex")
    });

    await foundZkAssetContract.callTx.proveOwnership();

  });
});

//  ❯ src/test/UT-M2.test.ts (1 test | 1 failed) 27464ms
// × API > UT-M2 · Proof Format Check 1910ms
// → failed assert: Not Owner