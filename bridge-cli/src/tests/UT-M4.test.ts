...
describe('API', () => {
    ...
    it('UT-M4 · Prove owneship of the asset', async () => {
        ...
        expect(state?.assetAmount).toEqual(BigInt(1));
        expect(Buffer.from(state?.assetOwner ?? '').toString('hex')).toEqual(bridge_address)
        expect(Buffer.from(state?.operator ?? '').toString('hex')).toEqual(Buffer.from(persistentHash(rt_type, [pad("operator:pk:", 32), Buffer.from("b02b7df9b3e2e4f3b1122e85d619f31682ce55c1741a358ee8d277c6c5605b80", "hex")])).toString('hex'))

        const foundZkAssetContract = await api.joinContract(providers, zkAssetContract.deployTxData.public.contractAddress, {
            assetOwnerSecretKey: Buffer.from(bridge_key, "hex")
        });

        const result = await foundZkAssetContract.callTx.proveOwnership();

        expect(result?.public.status).toEqual("SucceedEntirely");
        expect(result?.private.result).toEqual(true);
